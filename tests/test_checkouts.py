...
class DexDBTest(Case):
    def test_checkouts(self):
        try:
            ...
            library.db.commit()
            library.db.shelfcodes = Shelfcodes(library.db)

            books = {}

            for i in range(1, 9):
                library.catalog.add_from_dexline(f'AUTHOR<TITLE{i}<SERIES<P')
                titles = list(library.catalog.grep(f'^AUTHOR$<^TITLE{i}$'))
                title = titles[0]
                books[i] = title.books[0]

            c1 = Checkout(library.db, None, member_id=thor.id,
            ...
            checkouts = Checkouts(library.db, member_id=thor.id)
            ...
            c2 = Checkout(library.db, None, member_id=thor.id,
            ...
            book_checkouts = Checkouts(library.db, book_id=books[1].id)
            ...
            checkout_timestamp = today - datetime.timedelta(weeks=5)
            c4 = Checkout(library.db, None, member_id=thor.id,
                          checkout_stamp=checkout_timestamp,
                          book_id=books[2].id)
            c4.create()
            c4.lose()
